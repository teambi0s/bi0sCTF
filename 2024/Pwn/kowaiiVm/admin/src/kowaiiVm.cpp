...
class kowaiiVm
{
    ...
    public:
        ...
        #ifdef DEBUG
        friend ostream& operator<<(ostream &os, const kowaiiVm &vm)
        {
            for(int i = 0; i < MAX_REGS; i++) os << "[X" << i << "] => 0x" << hex << (u64)vm.ctx.regs->x[i] << dec << endl;
            os << "[PC] => 0x" << hex << (u64)vm.ctx.regs->pc << dec << endl;
            os << "[SP] => 0x" << hex << (u64)vm.ctx.regs->sp << dec << endl;
            return os << endl;
        }
        #endif
};
