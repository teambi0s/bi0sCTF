// Source obtained from: https://github.com/google/google-ctf/blob/main/2023/quals/rev-oldschool/source/oldschool.c
...
#define PRNG_SEED 0xDEADBEEF  // The seed for the LCG. This is used to generate the keystream.
...
struct _err_tok_2 {
    const std::array<uint8_t, len> keystream;
    std::array<uint8_t, len> encrypted;
    ...
    // ✅ Removed attribute, kept constexpr for compile-time execution
    template <size_t ... Indices>
    constexpr _err_tok_2(const char* str, std::index_sequence<Indices...>) :
        keystream{PseudoRandomByte<Indices, seed>::val ...},
        encrypted{XorEncr(str[Indices], Indices) ...} {}

    // ✅ Decryption must be runtime
    const char *decrypt() {
        for (size_t i = 0; i < len - 1; ++i) {
            encrypted[i] = XorDecr(encrypted[i], i);
        }
        encrypted[len - 1] = '\0';
        return reinterpret_cast<const char*>(encrypted.data());
    }
};
 ...
 */
#define OBF(str) []{                                 \
        constexpr _err_tok_2<sizeof(str), 0> os(\
            str,                                     \
